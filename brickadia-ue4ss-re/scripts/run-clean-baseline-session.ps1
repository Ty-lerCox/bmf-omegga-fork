param(
  ...
  [string]$ProofOut = 'C:\Users\tycox\OneDrive\Documents\GitHub\Brickadia\brickadia-ue4ss-re\probes\CL12960\output\baseline-proof.jsonl',
  ...
  [string]$DeployProofModScript = 'C:\Users\tycox\OneDrive\Documents\GitHub\Brickadia\brickadia-ue4ss-re\scripts\deploy-proof-mod.ps1',
  ...
  [string]$Map = 'Plate',
  [int]$Port = 7777,
  [string]$LookupProbeSet = '',
  [int]$LookupDelayMs = 1500,
  [switch]$UnwrapHookParams
)
...
$originalLookupProbeSet = $env:OMEGGA_UE4SS_BASELINE_LOOKUPS
$originalLookupDelayMs = $env:OMEGGA_UE4SS_BASELINE_LOOKUP_DELAY_MS
$originalUnwrapParams = $env:OMEGGA_UE4SS_BASELINE_UNWRAP_PARAMS
...
if (!(Test-Path $DeployProofModScript)) {
  throw "Missing proof mod deploy script: $DeployProofModScript"
}
...
try {
  if ([string]::IsNullOrWhiteSpace($LookupProbeSet)) {
    Remove-Item Env:OMEGGA_UE4SS_BASELINE_LOOKUPS -ErrorAction SilentlyContinue
  } else {
    $env:OMEGGA_UE4SS_BASELINE_LOOKUPS = $LookupProbeSet
  }
  $env:OMEGGA_UE4SS_BASELINE_LOOKUP_DELAY_MS = [string]$LookupDelayMs
  $env:OMEGGA_UE4SS_BASELINE_UNWRAP_PARAMS = if ($UnwrapHookParams) { '1' } else { '0' }

  & $DeployProofModScript -RuntimeModsDir $RuntimeModsDir -ProofOut $ProofOut -SourceBundle $Bundle | Out-Null
  Set-Content -Path $modsTxt -Value 'BaselineObjectProof : 1' -Encoding utf8
  @(
    [pscustomobject]@{ mod_name = 'BaselineObjectProof'; mod_enabled = $true }
  ...
  foreach ($modName in @('BaselineObjectProof', 'BaselineChatProof', 'OmeggaBridge', 'OmeggaBridgeProbe')) {
    Backup-EnabledState -ModName $modName
  }

  foreach ($modName in @('BaselineChatProof', 'OmeggaBridge', 'OmeggaBridgeProbe')) {
  ...
  $proofEnabledPath = Join-Path (Join-Path $RuntimeModsDir 'BaselineObjectProof') 'enabled.txt'
  ...
  [pscustomobject]@{
    ...
    LookupProbeSet = if ([string]::IsNullOrWhiteSpace($LookupProbeSet)) { 'none' } else { $LookupProbeSet }
    LookupDelayMs = $LookupDelayMs
    UnwrapHookParams = [bool]$UnwrapHookParams
  } | ConvertTo-Json -Compress
}
finally {
  if ($null -eq $originalLookupProbeSet) {
    Remove-Item Env:OMEGGA_UE4SS_BASELINE_LOOKUPS -ErrorAction SilentlyContinue
  } else {
    $env:OMEGGA_UE4SS_BASELINE_LOOKUPS = $originalLookupProbeSet
  }

  if ($null -eq $originalLookupDelayMs) {
    Remove-Item Env:OMEGGA_UE4SS_BASELINE_LOOKUP_DELAY_MS -ErrorAction SilentlyContinue
  } else {
    $env:OMEGGA_UE4SS_BASELINE_LOOKUP_DELAY_MS = $originalLookupDelayMs
  }

  if ($null -eq $originalUnwrapParams) {
    Remove-Item Env:OMEGGA_UE4SS_BASELINE_UNWRAP_PARAMS -ErrorAction SilentlyContinue
  } else {
    $env:OMEGGA_UE4SS_BASELINE_UNWRAP_PARAMS = $originalUnwrapParams
  }
