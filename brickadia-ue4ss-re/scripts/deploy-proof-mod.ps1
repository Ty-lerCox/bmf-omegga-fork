...
$sourceMod = Join-Path $workspaceRoot ("probes\{0}\BaselineObjectProof" -f $SourceBundle)
if (!(Test-Path $sourceMod) -and $SourceBundle -ne 'CL12960') {
  $sourceMod = Join-Path $workspaceRoot 'probes\CL12960\BaselineObjectProof'
}
$targetMod = Join-Path $RuntimeModsDir 'BaselineObjectProof'
$modsTxt = Join-Path $RuntimeModsDir 'mods.txt'
$modsJson = Join-Path $RuntimeModsDir 'mods.json'

if (!(Test-Path $sourceMod)) {
  throw "Proof mod source is missing: $sourceMod"
}
...
$contents = $contents -replace '__BASELINE_PROOF_OUT__', ($ProofOut -replace '\\', '\\')
Set-Content -Path $mainLua -Value $contents -NoNewline

$modsLines = @()
if (Test-Path $modsTxt) {
  $modsLines = Get-Content $modsTxt | Where-Object { $_ -notmatch '^\s*BaselineObjectProof\s*:' }
}
$modsLines += 'BaselineObjectProof : 1'
...
$baselineEntry = $modsJsonArray | Where-Object { $_.mod_name -eq 'BaselineObjectProof' } | Select-Object -First 1
if ($null -eq $baselineEntry) {
  $modsJsonArray += [pscustomobject]@{
    mod_name = 'BaselineObjectProof'
    mod_enabled = $true
  }
} else {
  $baselineEntry.mod_enabled = $true
}

$modsJsonArray | ConvertTo-Json -Depth 8 | Set-Content -Path $modsJson -Encoding utf8

Write-Host "Deployed BaselineObjectProof to $targetMod"
